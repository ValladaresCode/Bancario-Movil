...
/**
 * LOOKUP DE CUENTA POR NÚMERO (para escaneo de QR / autocompletar destino).
 * Devuelve solo campos públicos de recepción — NUNCA el saldo. Estados HTTP
 * diferenciados para que el cliente muestre el error correcto:
 *   400 número inválido · 404 no existe · 409 cuenta inactiva/bloqueada.
 */
export const lookupAccountByNumber = async (req, res) => {
    try {
        const { numeroCuenta } = req.params;

        if (!numeroCuenta || !/^\d{1,10}$/.test(numeroCuenta)) {
            return res.status(400).json({
                success: false,
