...
const swaggerDefinition = {
    ...
    tags: [
        { name: "Authentication", description: "Inicio de sesión, verificación de email y recuperación de contraseña" },
        { name: "Registration (Admin)", description: "Registro directo y aprobaciones por Administrador" },
        { name: "Registration (Public)", description: "Solicitudes de registro públicas (requieren aprobación de un admin)" },
        { name: "Profile", description: "Perfil del usuario y solicitudes de actualización" },
    ...
    paths: {
                            ...
                            "application/json": {
                                schema: {
                                    ...
                                    properties: {
                                        ...
                                        token: { type: "string", description: "JWT para usar como Bearer token en ambos servicios" },
                                        userDetails: {
                                            type: "object",
                                            properties: {
                                                ...
                                                role: { type: "string", enum: ["ADMIN_ROLE", "EMPLOYEE_ROLE", "USER_ROLE"], example: "USER_ROLE" }
                                            }
                                        },
        ...
        "/auth/verify-email": {
            post: {
                ...
                description: "Confirma la dirección de email usando el token enviado por correo.",
                requestBody: {
                    required: true,
                    content: {
            ...
            get: {
                ...
                description: "Variante del endpoint anterior para el enlace que llega al correo: `GET /auth/verify-email?token=...`",
                parameters: [
        ...
        "/auth/resend-verification": {
            post: {
                ...
                requestBody: {
                    required: true,
                    content: {
                        "application/json": { schema: { $ref: "#/components/schemas/AuthEmailRequest" } }
                    }
                },
                responses: {
        ...
        "/auth/forgot-password": {
            post: {
                ...
                description: "Envía un email con el token para restablecer la contraseña.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": { schema: { $ref: "#/components/schemas/AuthEmailRequest" } }
        ...
        "/auth/reset-password": {
            post: {
                ...
                requestBody: {
                    required: true,
                    content: {
                        "application/json": { schema: { $ref: "#/components/schemas/AuthResetPasswordRequest" } }
                    }
                },
                responses: {
        ...
        "/auth/register": {
            post: {
                ...
                description: "Crea la cuenta de inmediato y envía email de verificación. Requiere contraseña de mínimo 8 caracteres y teléfono de 8 dígitos.",
                requestBody: {
                    required: true,
                    content: {
                        "multipart/form-data": { schema: { $ref: "#/components/schemas/AuthRegisterRequest" } }
                    }
                },
                responses: {
        ...
        "/auth/signup-request": {
            post: {
                ...
                description: "El usuario NO queda creado: un administrador debe aprobar la solicitud con `POST /auth/signup-requests/{id}/approve`.",
                requestBody: {
                    required: true,
                    content: {
                        "multipart/form-data": { schema: { $ref: "#/components/schemas/AuthRegisterRequest" } }
        ...
        "/auth/signup-requests": {
            get: {
                tags: ["Registration (Admin)"],
                summary: "Listar solicitudes de registro pendientes (Requiere Rol Admin)",
                security: [{ bearerAuth: [] }],
                responses: {
        ...
        "/auth/signup-requests/{id}/approve": {
            post: {
                ...
                description: "Crea el usuario a partir de la solicitud y envía el token de verificación por email.",
                security: [{ bearerAuth: [] }],
                parameters: [
        ...
        "/auth/signup-requests/status/{email}": {
            get: {
                ...
                description: "Permite al solicitante saber si su cuenta ya fue creada (VERIFIED), sigue pendiente o fue rechazada. No requiere autenticación.",
                parameters: [
                    { name: "email", in: "path", required: true, schema: { type: "string", format: "email" } }
                ],
                responses: {
                    "200": { description: "Estado de la solicitud (VERIFIED, PENDING, REJECTED o NOT_FOUND)" }
                }
            }
        },
        // =====================================================
        ...
        "/auth/profile": {
            get: {
                ...
                security: [{ bearerAuth: [] }],
                responses: {
                    "200": { description: "Perfil del usuario (datos personales, foto, rol, estado de verificación)" },
        ...
        "/auth/profile/by-id": {
            post: {
                ...
                description: "Usado por otros servicios para resolver los datos de un usuario a partir de su userId.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": {
        ...
        "/users/me": {
            patch: {
                ...
                description: "Los cambios NO se aplican de inmediato: quedan como solicitud pendiente que un administrador debe aprobar con `POST /users/update-requests/{id}/approve`.",
                security: [{ bearerAuth: [] }],
                requestBody: {
                    content: {
                        "multipart/form-data": { schema: { $ref: "#/components/schemas/UserProfileUpdateRequest" } }
                    }
                },
                responses: {
        ...
        "/users/update-requests": {
            get: {
                tags: ["Users (Admin)"],
                summary: "Lista las solicitudes de actualización de perfil (Requiere Rol Admin)",
                security: [{ bearerAuth: [] }],
                parameters: [
        ...
        "/users/update-requests/{id}/approve": {
            post: {
                tags: ["Users (Admin)"],
                summary: "Aprobar solicitud de actualización de perfil (Requiere Rol Admin)",
                description: "Aplica los cambios al usuario. Si cambió el email, se envía un nuevo token de verificación.",
                security: [{ bearerAuth: [] }],
                parameters: [
                    { name: "id", in: "path", required: true, schema: { type: "string" } }
                ],
        ...
        "/users/update-requests/{id}/reject": {
            post: {
                tags: ["Users (Admin)"],
                summary: "Rechazar solicitud de actualización de perfil (Requiere Rol Admin)",
                security: [{ bearerAuth: [] }],
                parameters: [
                    { name: "id", in: "path", required: true, schema: { type: "string" } }
                ],
        ...
        "/users/{userId}/role": {
            put: {
                ...
                security: [{ bearerAuth: [] }],
                parameters: [
                    { name: "userId", in: "path", required: true, schema: { type: "string" }, example: "user_regular_5" }
                ],
                requestBody: {
                    required: true,
                    content: {
        ...
        "/users/{userId}/roles": {
            get: {
                ...
                security: [{ bearerAuth: [] }],
                parameters: [
                    { name: "userId", in: "path", required: true, schema: { type: "string" }, example: "user_premium_1" }
                ],
                responses: {
                    "200": {
                        ...
                        content: {
                            "application/json": {
                                schema: { type: "array", items: { type: "string" }, example: ["USER_ROLE"] }
                            }
                        }
                    },
                    "401": { description: "Token inválido o no provisto" }
        ...
        "/users/by-role/{roleName}": {
            get: {
                tags: ["Users (Admin)"],
                summary: "Lista los usuarios que tienen un rol específico (Requiere Rol Admin)",
                security: [{ bearerAuth: [] }],
                parameters: [
                    { name: "roleName", in: "path", required: true, schema: { type: "string", enum: ["ADMIN_ROLE", "EMPLOYEE_ROLE", "USER_ROLE"] } }
                ],
                responses: {
        ...
        schemas: {
            AuthRegisterRequest: {
                ...
                properties: {
                    ...
                    phone: { type: "string", example: "12345678", description: "8 dígitos" },
            ...
            AuthLoginRequest: {
                ...
                properties: {
                    email: { type: "string", format: "email", example: "premium1@test.com" },
