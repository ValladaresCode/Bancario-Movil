...
export const CurrencyDashboard = ({ allowedCurrencies }) => {
    ...
    return (
        <section className="space-y-6">
            {/* Control Panel */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-6 rounded-[2rem] border border-[var(--theme-border)] bg-[var(--theme-surface)] shadow-lg backdrop-blur-xl">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                    <div className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl bg-[var(--theme-surface-alt)] border border-[var(--theme-border)] flex-1 sm:flex-none min-w-0">
                        <span className="text-sm font-bold text-[var(--theme-text-muted)] shrink-0">{base}</span>
                        <input 
                            type="number" 
                            value={amount}
                            onChange={(e) => setAmount(Number(e.target.value))}
                            className="w-full min-w-[60px] bg-transparent text-[var(--theme-text)] font-bold focus:outline-none"
                    ...
                    <select 
                        ...
                        className="flex-1 sm:flex-none min-w-0 px-3 sm:px-4 py-2 rounded-xl bg-[var(--theme-surface-alt)] border border-[var(--theme-border)] text-[var(--theme-text)] focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm"
                    >
                    ...
                    <button 
                        onClick={fetchRates}
                        disabled={loading}
                        className="p-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white transition-all disabled:opacity-50 group shrink-0"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {displayedRates.map(([code, rate]) => (
                    <div 
                        key={code}
                        className="group relative overflow-hidden p-4 sm:p-6 rounded-[2rem] border border-[var(--theme-border)] bg-[var(--theme-surface)] hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
                    >
                        ...
                        <div className="flex flex-col gap-3 sm:gap-0 sm:flex-row sm:justify-between sm:items-start">
                            <div className="flex items-center gap-3 sm:gap-4">
                                <div className="p-2 sm:p-3 rounded-2xl bg-blue-500/10 text-blue-500 ring-1 ring-blue-500/20 shrink-0">
                                    {CURRENCY_ICONS[code] || <Globe className="w-5 h-5" />}
                                </div>
                                <div className="min-w-0">
                                    <h4 className="text-lg sm:text-xl font-bold text-[var(--theme-text)]">{code}</h4>
                                    <p className="text-[10px] sm:text-xs font-medium text-[var(--theme-text-muted)] uppercase tracking-wider truncate">
                                        {CURRENCY_NAMES[code] || 'Divisa'}
                                    </p>
                                </div>
                            </div>
                            <div className="text-left sm:text-right">
                                <span className="text-xl sm:text-2xl font-black text-[var(--theme-text)] tracking-tight break-all">
                        ...
                        <div className="mt-4 sm:mt-6 w-full h-1.5 bg-[var(--theme-surface-alt)] rounded-full overflow-hidden">
                            <div 
